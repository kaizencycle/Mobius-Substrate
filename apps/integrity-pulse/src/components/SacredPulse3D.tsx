...
export function FractalSpheres({ mii = 0.98, eventsPerMin = 1 }: { mii?: number; eventsPerMin?: number }) {
  const mesh = useRef<InstancedMesh>(null!);
  const temp = new Object3D();
  const N = 450;
  const color = new Color().setHSL(0.33 + 0.1 * mii, 0.9, 0.55);
...
export default function SacredPulse3D({ mii, eventsPerMin }: { mii: number; eventsPerMin: number }) {
  return (
    <Canvas camera={{ position: [0, 0, 18], fov: 55 }}>
      ...
      <FractalSpheres mii={mii} eventsPerMin={eventsPerMin} />
      <EffectComposer>
