...
export default function PulseDemo(){
  ...
  const { mii, entropy, eventsPerMin } = usePulseFeed(process.env.NEXT_PUBLIC_PULSE_API);
  return (
    <div style={{minHeight:'100vh', background:'#0b1d12', color:'white'}}>
      ...
      <div style={{height:'calc(100vh - 68px)', padding:12}}>
        {mode==='svg' ? (
          <IntegrityPulseSVG mii={mii} entropy={entropy} eventsPerMin={eventsPerMin}/>
        ) : (
          <SacredPulse3D mii={mii} eventsPerMin={eventsPerMin}/>
        )}
      </div>
    </div>
  );
