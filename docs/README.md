# 📚 Mobius Systems Documentation

> **Complete documentation for Mobius Systems — the world's first KTT-native operating system.**

---

## 🚀 Quick Start

**New here?** Start with **[00-START-HERE.md](./00-START-HERE.md)** for guided onboarding.

**Looking for something specific?** Use the navigation below or search (Ctrl+F).

---

## 📖 Documentation Categories

### 01. 📚 Whitepapers — Economic Theory
> *Complete economic specifications for the Mobius Integrity Credits (MIC) system.*

**[📂 Browse Whitepapers](./01-whitepapers/)**

- [MIC Whitepaper v2.0](./01-whitepapers/MIC_Whitepaper_v2.0.md) — Complete shard-denominated specification ⭐
- [UBI Mechanism v2.0](./01-whitepapers/UBI_Mechanism_v2.0.md) — MII-throttled distribution
- [Foundation Economics](./01-whitepapers/MIC_Foundation_Up_Economics_Addendum.md) — Foundation-up philosophy

**For**: Economists, researchers, policy makers

---

### 02. 🏛️ Governance — Foundation Structure
> *Governance structure, processes, and decision-making frameworks.*

**[📂 Browse Governance](./02-governance/)**

- [Overview](./02-governance/overview.md) — Two-house model (Council + TSC) ⭐
- [Concord Council](./02-governance/council.md) — 7-seat ethics & policy body
- [TSC](./02-governance/tsc.md) — 5-seat technical leadership
- [Elections](./02-governance/elections.md) — Ranked-choice voting, meritocratic selection
- [Quorum & Voting](./02-governance/quorum.md) — Voting rules, deadlock resolution

**For**: Governance participants, community members

---

### 03. 🏗️ Architecture — System Design
> *Technical architecture, design patterns, and system blueprints.*

**[📂 Browse Architecture](./03-architecture/)**

- [Labs Master Architecture](./03-architecture/technical/LABS_MASTER_ARCHITECTURE.md) — Complete system architecture
- [Technical Docs](./03-architecture/technical/) — Bridge architecture, interface layer
- [Diagrams](./03-architecture/diagrams/) — Visual architecture representations
- [ADRs](./03-architecture/adr/) — Architecture Decision Records
- [Mobius Local Node](./nodes/MOBIUS_LOCAL_NODE.md) — Offline Sentinel stack using local LLMs

**For**: Architects, senior developers, system designers

---

### 04. 📖 Guides — Practical How-To
> *User and developer guides for deploying, developing, and operating Mobius.*

**[📂 Browse Guides](./04-guides/)**

#### Quickstart
- [START HERE](./04-guides/quickstart/START_HERE.md) — New user onboarding ⭐

#### Deployment
- [Deployment Guide](./04-guides/deployment/DEPLOYMENT_GUIDE.md) — General deployment
- [Vercel Deployment](./04-guides/deployment/VERCEL_DEPLOYMENT_GUIDE.md) — Vercel-specific
- [Civic Mount Integration](./04-guides/deployment/CIVIC_MOUNT_INTEGRATION.md) — Infrastructure integration

#### Development
- [Frontend Development](./04-guides/development/FRONTEND_DEVELOPMENT.md) — UI/UX development
- [API Integration](./04-guides/development/API_INTEGRATION.md) — API connections
- [Cursor Integration](./04-guides/development/cursor-integration.md) — AI editor setup

#### Operations
- [Custodian Guide](./04-guides/operations/CUSTODIAN_GUIDE.md) — System maintenance
- [Recovery Playbook](./04-guides/operations/RECOVERY_PLAYBOOK.md) — Disaster recovery
- [Forking Guide](./04-guides/operations/FORKING_GUIDE.md) — How to fork

**For**: New users, developers, operators, DevOps

---

### 05. 🔬 Research — Academic Foundations
> *Academic research, theoretical frameworks, and foundational principles.*

**[📂 Browse Research](./05-research/)**

#### Foundations
- [Foundational Blueprint v2](./05-research/foundations/foundational_blueprint_integrity_economy_v2.md) — Complete theoretical framework
- [Kaizen Theorems](./05-research/foundations/kaizen_theorems.md) — Mathematical axioms (KZ-Θ series)
- [Civic Transition Cycle](./05-research/foundations/civic_transition_cycle.md) — 5-phase adoption framework

#### Constitution
- [AI Integrity Constitution](./05-research/constitution/AI_INTEGRITY_CONSTITUTION.md) — AI alignment principles
- [Independence Manifest](./05-research/constitution/INDEPENDENCE_MANIFEST.md) — Sovereignty declaration

#### KTT
- [Kaizen Turing Test](./05-research/ktt/) — Evaluation framework

**For**: Researchers, academics, theorists

---

### 06. ⚙️ Specifications — Technical Specs
> *Formal technical specifications, API docs, and protocol definitions.*

**[📂 Browse Specifications](./06-specifications/)**

- [APIs](./06-specifications/apis/) — REST, GraphQL, WebSocket specs
- [Protocols](./06-specifications/protocols/) — Network & communication protocols
- [Manifests](./06-specifications/manifests/) — Configuration specifications

**For**: Integration developers, protocol implementers

---

### 07. 📚 Reference — Quick Lookup
> *Indices, registries, and navigation aids for quick reference.*

**[📂 Browse Reference](./07-reference/)**

- [Document Registry](./07-reference/DOCUMENT_REGISTRY.md) — Complete doc inventory
- [Repository Structure](./07-reference/REPOSITORY_STRUCTURE.md) — Repo organization
- [Master Index](./07-reference/INDEX.md) — Comprehensive topic index

**For**: Everyone looking for quick lookups

---

### 08. 🔄 Processes — Workflows & Runbooks
> *Operational procedures, runbooks, rituals, and process documentation.*

**[📂 Browse Processes](./08-processes/)**

- [Rituals](./08-processes/rituals/) — Community ceremonies, governance rituals
- [Runbooks](./08-processes/runbooks/) — Step-by-step procedures
- [Operations](./08-processes/operations/) — Standard operating procedures

**For**: Operators, community organizers

---

### 09. 📊 Reports — Status & Analysis
> *Progress reports, implementation summaries, and status documentation.*

**[📂 Browse Reports](./09-reports/)**

- [Implementation Reports](./09-reports/implementation/) — Feature completion status
- [Cycle Reports](./09-reports/cycles/) — Cycle-specific progress
- [Communications](./09-reports/communications/) — Public announcements
- [Executive Presentation](./09-reports/EXECUTIVE_PRESENTATION.md) — Stakeholder summary

**For**: Stakeholders, community members, contributors

---

### 🗄️ Archive — Historical Docs
> *Deprecated and historical documentation.*

**[📂 Browse Archive](./archive/)**

- Completed initiatives
- Deprecated specifications
- Historical records

---

## 🎯 Common Tasks

| I want to... | Go to... |
|--------------|----------|
| **Understand MIC economics** | [01-whitepapers/MIC_Whitepaper_v2.0.md](./01-whitepapers/MIC_Whitepaper_v2.0.md) |
| **Learn governance** | [02-governance/overview.md](./02-governance/overview.md) |
| **Deploy Mobius** | [04-guides/deployment/DEPLOYMENT_GUIDE.md](./04-guides/deployment/DEPLOYMENT_GUIDE.md) |
| **Develop features** | [04-guides/development/](./04-guides/development/) |
| **Understand architecture** | [03-architecture/](./03-architecture/) |
| **Read research** | [05-research/](./05-research/) |
| **Find a document** | [07-reference/DOCUMENT_REGISTRY.md](./07-reference/DOCUMENT_REGISTRY.md) |
| **Contribute** | [../CONTRIBUTING.md](../CONTRIBUTING.md) |
| **Report security issue** | [../.github/SECURITY.md](../.github/SECURITY.md) |

---

## 🔍 Finding Information

### Navigation Tips

1. **Browsing?** Start with category READMEs (e.g., [01-whitepapers/README.md](./01-whitepapers/README.md))
2. **Searching?** Use Ctrl+F or GitHub's search
3. **Lost?** Return to [00-START-HERE.md](./00-START-HERE.md)
4. **Need overview?** See [07-reference/](./07-reference/)

### By Role

- **New User**: [00-START-HERE.md](./00-START-HERE.md) → [04-guides/quickstart/](./04-guides/quickstart/)
- **Developer**: [04-guides/development/](./04-guides/development/) → [03-architecture/](./03-architecture/)
- **Researcher**: [01-whitepapers/](./01-whitepapers/) → [05-research/](./05-research/)
- **Operator**: [04-guides/operations/](./04-guides/operations/) → [08-processes/](./08-processes/)
- **Governance**: [02-governance/](./02-governance/) → [Foundation Docs](../FOUNDATION/)

---

## 📂 Repository Structure

```
Mobius-Systems/
├── FOUNDATION/                 # Foundation governance & policies
│   ├── CHARTER.md
│   ├── BYLAWS.md
│   ├── POLICIES/
│   ├── LICENSES/
│   ├── LEGAL/
│   └── PROCESS/
│
├── docs/                       # All documentation (you are here)
│   ├── 00-START-HERE.md       # Onboarding entry point
│   ├── 01-whitepapers/        # Economic theory
│   ├── 02-governance/         # Foundation governance
│   ├── 03-architecture/       # System design
│   ├── 04-guides/             # How-to guides
│   ├── 05-research/           # Academic research
│   ├── 06-specifications/     # Technical specs
│   ├── 07-reference/          # Quick reference
│   ├── 08-processes/          # Workflows
│   ├── 09-reports/            # Status reports
│   └── archive/               # Historical docs
│
├── apps/                       # Application code
├── packages/                   # Shared packages
├── sentinels/                  # AI governance agents
├── labs/                       # Lab implementations
├── monitoring/                 # Observability stack
└── .github/                    # GitHub configs, templates
```

---

## 📞 Getting Help

### Documentation Issues
- 🐛 [Report doc issue](https://github.com/kaizencycle/Mobius-Systems/issues)
- 💬 [Ask in discussions](https://github.com/kaizencycle/Mobius-Systems/discussions)

### Community Support
- **General**: community@mobius.systems
- **Technical**: support@mobius.systems
- **Governance**: governance@mobius.systems
- **Security**: security@mobius.systems

### Contributing
- 🤝 [Contribution Guide](../CONTRIBUTING.md)
- 📝 [RFC Process](../FOUNDATION/PROCESS/RFC_PROCESS.md)
- 🛡️ [Code of Conduct](../FOUNDATION/POLICIES/CODE_OF_CONDUCT.md)

---

## 🔄 Documentation Updates

This documentation is version-controlled. Recent major changes:

- **2025-11-08**: Complete docs reorganization (numbered categories, comprehensive READMEs)
- **2025-11-08**: Foundation v2 documentation added
- **2025-11-08**: Whitepaper rebrand (GIC→MIC, GI→MII)
- **2025-11-07**: Monitoring stack documentation
- **2025-11-07**: Governance structure finalized

See [CHANGELOG.md](../CHANGELOG.md) for detailed change history.

---

## 📜 License

Documentation licensed under **CC-BY-SA-4.0** unless otherwise noted.

Code and implementations follow **AGPL-3.0** + [Ethical Addendum](../FOUNDATION/LICENSES/LICENSE-Ethical-Addendum.md).

---

## 🌀 Mobius Systems

*"Intelligence moves. Integrity guides."*

**[⬆️ Back to Top](#-mobius-systems-documentation)** | **[🏠 Main README](../README.md)** | **[🚀 Get Started](./00-START-HERE.md)**
