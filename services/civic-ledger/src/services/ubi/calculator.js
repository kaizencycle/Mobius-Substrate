...
export function calculateUBI(input, config) {
    ...
    const U = (α * I + β * Re) / 1000000n + D;
    // 2) caps κ, σ
    const capRes = (reservesShards * BigInt(Math.floor(caps.max_share_of_reserves * 1e6))) / 1000000n;
    const capCirc = (circulatingShards * BigInt(Math.floor(caps.max_share_of_circulating * 1e6))) / 1000000n;
    ...
    let g;
    if (GI >= gi_thresholds.bonus.min)
        g = gi_thresholds.bonus.g;
    else if (GI >= gi_thresholds.normal.min)
        g = gi_thresholds.normal.g;
    else if (GI >= gi_thresholds.throttle.min)
        g = gi_thresholds.throttle.g;
    else
        return {
