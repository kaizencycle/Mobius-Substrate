...
export function toLD(beacon: unknown) {
  return JSON.stringify(beacon);
}

export function beaconFromRankedItem(item: { id?: string; "@type"?: string; url?: string; name?: string; description?: string; dateModified?: string; keywords?: string[]; oaa?: unknown }) {
  return {
    id: item.id,
